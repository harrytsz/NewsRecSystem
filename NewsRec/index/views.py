...
def home(request):
    ...
    total = 0 # 总页数
    # 如果 cate 是为你推荐，走该部分逻辑 tag_flag = 0 表示不是从标签召回数据
    if _cate == "1":
...
def getRecNews(request):
    ...
    if tag_flag == 1 and int(baseclick) == 0:
        ...
        for tag in tags_list:
            ...
            return new.objects.filter(new_id__in=new_id_list)[:20], news_id_hot_dict
    # 走正常排序逻辑
    elif tag_flag == 0:
        # 首先判断用户是否有浏览记录
        # 如果又该用户的浏览记录，则从浏览的新闻获取相似的新闻返回
        if newbrowse.objects.filter(user_name=uname).exists():
            # 判断用户最忌浏览的新闻是否够 10 个，如果够的话， 取 top 10,每个取两个相似
            # 如果不够 10 个， 每个取 20 / 真实个数 + 1 相似
            num = 0
        ...
        # 如果该用户没有浏览记录，即该用户是第一次进入系统且没有选择任何标签，返回热度榜单数据的 20 - 40
        else:
            # 从新闻热度表中取 top20 新闻数据
...
def switchuser(request):
    if "username" in request.session.keys():
        ...
        print("用户： %s 执行了切换用户动作，删除其对应的 session 值 ...." % uname)
